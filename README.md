...
ランキング機能（出勤日数ランキングと滞在時間ランキング）は以下のタイミングでのみ更新されます：

1. ページの初回読み込み時
2. ランキングコンポーネント（AttendanceRanking）のマウント時
3. アプリを再起動したとき

**注意事項:**
- ランキングは学生の出退勤操作を行なった直後には自動更新されません
- 最新の順位を反映させるにはページをリロードする必要があります
- これは処理負荷を軽減し、UIのパフォーマンスを優先するための設計です

```mermaid
flowchart TD
    A[ページ読み込み] --> B[CSVデータ読み込み]
    B --> C[ローカルストレージ読み込み]
    C --> D[学生ごとのデータ集計]
    D --> E[出勤日数ランキング作成]
    D --> F[滞在時間ランキング作成]
...
同点の場合のランキング表示優先順位：
1. 数値（出勤日数/滞在時間）が多い順
2. 学年の優先度（教員 > M2 > M1 > B4）
3. 学生の登録順序

## 年間出勤カレンダーの最適化

年間出勤カレンダーは全月のデータを一度に表示すると、パフォーマンスに影響が出るため、以下の最適化を実装しています。

### 表示の最適化

```mermaid
flowchart TD
    A[カレンダーマウント時] --> B[現在月の前後2ヶ月だけ表示]
    B --> C{ユーザーのスクロール}
    C -->|下方向へスクロール| D[次の月のデータを動的に読み込み・表示]
    C -->|上方向へスクロール| E[前の月のデータを動的に読み込み・表示]
    D --> C
    E --> C
```
...
flowchart TD
    A[月データ要求] --> B{キャッシュに存在?}
    B -->|Yes| C[キャッシュからデータ取得]
    B -->|No| D[CSVファイルから読み込み]
    D --> E[データをパース・処理]
    E --> F[キャッシュに保存]
    F --> G[データを返却]
    C --> G
...
3. **メモ化によるレンダリング最適化**
   ...
   - useCallbackとuseMemoによる関数と計算結果の再利用
   - 不必要な再計算・再レンダリングを防止

4. **スクロール最適化**
   - ビューポート外のコンテンツはレンダリングしない
   - スクロール検知の効率化（スロットリング）

これらの最適化により、特に学生数が多い環境でのパフォーマンスが大幅に向上しています。
...
<tr>
<td>ページ読み込み<br/>ランキングコンポーネントマウント</td>
<td>
• エクスポートパスの確認<br>
• CSVファイルから過去の出勤日・滞在時間を読み込み<br>
• LocalStorageから現在の出勤状態を読み込み<br>
• 学生ごとのデータを集計<br>
• ランキングデータの計算と並び替え<br>
• トップ5の学生を表示
</td>
